...
## 결정 기록 (Decisions — 2026-06-28 확정)

> 위 Open Questions에 대해 휴먼(C.H)이 확정한 결정. 이후 `DESIGN-SPEC.md`와 구현은 이 값을 기준으로 한다.

**설계 핵심**
- **D1 (Q1) 줄바꿈 기준:** Frontend가 실제 글꼴 픽셀 폭을 측정해 줄바꿈. `core/paginate.ts`는 글자수 기준 1차 분할만 하고, Frontend가 글꼴 로드 후 재측정·보정한다. 글자 크기 −/+ 변경 시 `glyphX` 재측정 → 타임라인 재빌드.
- **D2 (Q2) 스윕 바 이동:** 한 줄씩 좌→우 이동, 줄 끝에서 다음 줄 시작점으로 하강. (페이지 전체 훑기 아님.)
- **D3 (Q4) 타이머 정의:** 활성 읽기 시간 기준. 일시정지·휴식 중에는 타이머·스윕 바 모두 정지. `totalMs` 도달 시 타이머 우선 종료.
- **D4 (Q5) 글 끝+시간 남음:** 같은 글을 처음부터 반복.

**리소스·프로세스**
- **D5 (Q8) 글꼴:** Pretendard 동봉 (OFL, 상업·동봉 자유, 받침 가독성 우수). 기본 24pt↑.
- **D6 (Q9) 명언:** 퍼블릭 도메인(속담·고전) 위주 수집 → 10세 눈높이 쉬운 우리말 다듬기. 부족분은 속담 각색·자체 창작으로 보충. `source`·`license` 컬럼 필수 기록.
- **D7 (실행 방식):** Phase 1 이후 구현은 Workflow 병렬 파이프라인으로 진행(Phase 2·3·7 동시). 게이트는 휴먼 승인.

**기술·컴플라이언스**
- **D8 (Q6) 네이티브 모듈:** Phase 1에서 `electron-rebuild`로 better-sqlite3 ABI 검증을 먼저 통과시켜 Phase 8 리스크 제거.
- **D9 (Q7) 빌드 환경:** 현재 Windows 환경에서 직접 `.exe` 빌드·스모크 테스트. CI는 선택 사항.
- **D10 (Q10) 아동 개인정보:** **실명을 받지 않고 별명 + 아바타만 사용.** 프로필 등록은 별명 입력 + 아바타 선택으로 구성(`profiles.name`은 별명 용도). 모든 데이터는 로컬 SQLite에만 저장, 외부 전송 없음. 프로필/종료 화면에 명시.
